...
(* AST を畳んだ、平らな命令の列。
   ...
   ここは何にも依存しない。**書く側だけが、ここに居ます** -- 読む側は Rust
   (tsbvm/src/tsb.rs)で、この形だけを見て話す。OCaml は AST を組んで畳むところ
   まで、Rust は走らせるところ。境界は、このファイルの形そのもの。
   ...
   畳めない形に出会ったら Tocode が「まだ畳めない」と言って止まる(黙って別の
   道を通ったりしない -- 走らせる側に parser が無い以上、あとで落ちるより、
   ここで言ったほうがいい)。 *)
